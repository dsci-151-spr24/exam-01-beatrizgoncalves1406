...
*I, \_B\_E\_A\_T\_R\_I\_Z\_\_\_\_\_, hereby state that I have not communicated with or gained information in any way from my classmates or anyone other than the Professor or TA during this exam, and that all work is my own.*
...
1.  **Question 1 (10 points)** - What are the ten most common destinations for flights from NYC airports in 2013?***The ten most common destinations are "ORD", "ATL", "LAX", "BOS", "MCO", "CLT", "SFO", "FLL", "MIA", and "DCA".***
...
***The airlines with the most flights departing from NYC airports are United Air Lines Inc., JetBLue Airways, and ExpressJet Airlines.***
...
3.  **Question 3 (10 points)** - Consider only flights that have non-missing arrival delay information.
    ...
    a\.
    Which carrier had the highest mean arrival delay?***The Frontier Airlines Inc. has the highest mean arrival delay of 21.9207048 minutes.***

    b\.
    Which carrier had the lowest mean arrival delay? ***The Alaska Airlines Inc. has the lowest mean arrival delay of -9.9308886 minutes.***
```{r}
flights %>%
  group_by(carrier) %>%
  ...
  arrange(desc(delay))
```
4.  **Question 4 (10 points)** - What was the mean temperature at the origin airport on the day with the highest departure delay?***The highest departure delay # 1301 minutes while on the flight to JFK on January 9th. The mean temperature for that day # 42.6575 degrees.***
...
6.  **Question 6 (15 points)** - Find the flight with the longest air time.
    ...
    a\.
    How long is this flight?***The flight # 695 minutes long.***

    b\.
    What city did it fly to?***The city it flew into # Honululu.***

    c\.
    How many seats does the plane that flew this flight have?***The plane on this flight has 292 seats.***
```{r}
flights %>%
  count(air_time, tailnum, dest) %>%
...
7.  **Question 7 (15 pts)** - The `airports` data frame contains information on a large number of primarily American airports.
    ...
    What does this tell you about the geographical distribution of these airports?***It tells my that the Northern point has 48 contiguous states with a lat of approx 49.38 and a lon of approx -95.15. The Southern point has 48 contiguous states with a lat approx 24.5 and a lon of approx -81.96. The Western point has 48 contiguous states with a lat of approx 48.17 and a lon of approx -124.77. The Eastern point is in the 50 states and has a lat of approx 44.81 and a lon of approx -66.94.***
    *Hint:* You will first need to limit your analysis to the Contiguous United States.
    ...
    Make sure to cite whatever resource you use.***I used the notes, the Wikipedia article, and this website called www.veracalendario.info***
```{r}
airports %>%
  filter(
    lat > 24.520833333333332 & lat < 49.38447222222222, lon > -124.7716944444445 & -66.9470277777778
  ) %>%
  ggplot(aes(x = lon)) +
...
flights %>%
  ...
  geom_boxplot() +
  labs(title = "NYC flights that are on time",
